...
class ESRGANOptimized:
    def __init__(self, model_path, tile_size=256, model_input_size=128, prepad=0, scale=4, num_threads=2, overlap_size=32):
        ...
        self.model_output_size = model_input_size * scale
        self.prepad = prepad
        self.scale = scale
        ...
        self.infer_requests = [self.session.create_infer_request() for _ in range(self.num_threads)]

    def _init_model(self):
        ie = ov.Core()
        self.model_onnx = ie.read_model(model=self.model_path)
        self.session = ie.compile_model(model=self.model_onnx)
    ...
    def _process_tile_worker(self):
        ...
        while True:
            ...
            try:
                # Get an available Infer Request
                infer_request = self.infer_requests[idx % len(self.infer_requests)]

                # Asynchronously submit the inference request
                infer_request.start_async(inputs=[tile])
                infer_request.wait()
                result = infer_request.output_tensors[0].data[0]
...
def main():
    model_path = '4xNomos2_realplksr_dysample_256_fp32_fullyoptimized.xml'
